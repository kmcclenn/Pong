...
float player_difficulty = 0.04;
float computer_difficulty = 0.1;
int[] score = {0, 0};
...
void draw() {
  ...
  rect_1.move(player_difficulty * distance_mouse_rect);
  float distance_rect_square = square.location.x + square.side/2 - (rect_2.x + rect_2.w/2);
  rect_2.move(computer_difficulty * distance_rect_square);



  if (((square.location.y + square.side) > rect_2.y) && (square.location.y < rect_2.y + rect_2.h) && (square.location.x + square.side > rect_2.x) && (square.location.x < (rect_2.x + rect_2.w))) {
  ...
  if (((square.location.y) < (rect_1.y + rect_1.h)) && (square.location.y > rect_1.y) && (square.location.x + square.side > rect_1.x) && (square.location.x < (rect_1.x + rect_1.w))) {
    ...
    square.speed.x = cos(angle) * magnitude;
    square.speed.y = sin(angle) * magnitude;
  } else if (((square.location.x + square.side) > rect_1.x) && ((square.location.x) < rect_1.x) && (square.location.y + square.side > rect_1.y) && (square.location.y < (rect_1.y + rect_1.h))) {
    square.speed.x = square.speed.x * -1;
  } else if (((square.location.x) < rect_1.x + rect_1.w) && ((square.location.x) > rect_1.x + rect_1.w/2) && (square.location.y + square.side > rect_1.y) && (square.location.y < (rect_1.y + rect_1.h))) {
    square.speed.x = square.speed.x * -1;
  }
